...
function assert(markdown, expectedHtml) {
...
describe('parseMarkdownString', () => {
  ...
  test('should parse mentions', async () => {
    assert(
      ...
      '<label>@mention</label> at the beginning',
    );
    assert(
      'This is a @mention in the middle',
      'This is a <label>@mention</label> in the middle',
    );
    assert(
      'At the end is a @mention',
      'At the end is a <label>@mention</label>',
    );
